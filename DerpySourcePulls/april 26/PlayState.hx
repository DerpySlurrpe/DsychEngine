...
//DERPY CHECKMARK NOT FINISHED
	...
	override public function create()
	{
		...
		var file:String = Paths.json(songName + '/dialogue'); //Checks for json/Slush Engine dialogue
		if (OpenFlAssets.exists(file)) {
			dialogueJson = DialogueBoxSlush.parseDialogue(file);
		}
		...
		timeTxt.setFormat(Paths.font("next.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("typos.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "BuddyBot Mode", 32);
		botplayTxt.setFormat(Paths.font("next.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
// opponet workshop 
	...
	public var slushDialogue:DialogueBoxSlush;
	//You don't have to add a song, just saying. You can just do "startDialogue(dialogueJson);" and it should work
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		// TO DO: Make this more flexible, maybe?
		if(slushDialogue != null) return;

		if(dialogueFile.dialogue.length > 0) {
			inCutscene = true;
			CoolUtil.precacheSound('dialogue');
			CoolUtil.precacheSound('dialogueClose');
			slushDialogue = new DialogueBoxSlush(dialogueFile, song);
			slushDialogue.scrollFactor.set();
			if(endingSong) {
				slushDialogue.finishThing = function() {
					slushDialogue = null;
					endSong();
				}
			} else {
			...
			slushDialogue.nextDialogueThing = startNextDialogue;
			slushDialogue.skipDialogueThing = skipDialogue;
			slushDialogue.cameras = [camHUD];
			add(slushDialogue);
		} else {
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Funked Up Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
