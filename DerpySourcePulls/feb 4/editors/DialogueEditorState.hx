...
class DialogueEditorState extends MusicBeatState
{
	...
	function updateTextBox() {
		...
		DialogueBoxSlush.updateBoxOffsets(box);
	}
	...
	function reloadCharacter() {
		...
		character.x = DialogueBoxSlush.LEFT_CHAR_X;
		character.y = DialogueBoxSlush.DEFAULT_CHAR_Y;

		switch(character.jsonFile.dialogue_pos) {
			case 'right':
				character.x = FlxG.width - character.width + DialogueBoxSlush.RIGHT_CHAR_X;
	...
	function reloadText(speed:Float = 0.05) {
		...
		daText = new Alphabet(DialogueBoxSlush.DEFAULT_TEXT_X, DialogueBoxSlush.DEFAULT_TEXT_Y, textToType, false, true, speed, 0.7);
		add(daText);
		...
		DiscordClient.changePresence("In The Dialogue Editor", rpcText);
		#end
	}
			...
			if(sender == characterInputText) {
				...
				if(character.jsonFile.animations.length > 0) {
					...
					if(character.jsonFile.animations.length > curAnim && character.jsonFile.animations[curAnim] != null) {
						...
						animText.text = 'Animation: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Press W or S to scroll around';
					} else {
						animText.text = 'ERROR! NO ANIMATIONS FOUND';
					}
					characterAnimSpeed();
