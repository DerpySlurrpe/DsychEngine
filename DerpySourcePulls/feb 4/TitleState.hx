...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.play(StartTime:Float = 12.00,);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxG.sound.play(Paths.sound('confirmTitle'), 0.5);

				transitioning = true;
				// FlxG.sound.music.stop();
				
				FlxTween.tween(FlxG.camera, {y: FlxG.height}, 1.8, {ease: FlxEase.expoIn, startDelay: 0.1});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
				...
				case 1:
					#if SLUSH_WATERMARKS
					createCoolText(['Slush Engine by'], 15);
					#else
				...
				case 3:
					#if SLUSH_WATERMARKS
					addMoreText('DerpySlurrpe', 15);
					addMoreText('SlushiiGod', 15);
					#else
				...
				case 4:
					deleteCoolText();
				// credTextShit.visible = false;
				// credTextShit.text = 'Dripified with love';
				// credTextShit.screenCenter();
				case 5:
					#if SLUSH_WATERMARKS
					createCoolText(['Best to stayed dripped', 'with'], -40);
					#else
					createCoolText(['The one and only', 'with'], -40);
					#end
				case 7:
					addMoreText('', -40);
					ngSpr.visible = false;
				// credTextShit.text += '\Here we go now';
				case 8:
					deleteCoolText();
				//addMoreText

				// credTextShit.text = 'Shoutout to Slush';
				// credTextShit.screenCenter();
				case 9:
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxG.camera.flash(FlxColor.WHITE, 1.28);
			remove(credGroup);
			skippedIntro = true;
		}
	}
